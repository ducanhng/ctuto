/*Ex 6_4: Write a cross-referencer that prints a list of all words in a document, and, for each word, a list of the line numbers on which it occurs. Remove noise words like "the", "and," and so on.*/
...
int treecount(struct tnode *);
struct tnode **addlist(struct tnode **, struct tnode *);
void listprint(struct tnode **list, int lim);
int comp_count(struct tnode *,struct tnode *);
...
int main(int argc, char *argv[])
{
	...
	treeprint(root);
	printf("--------------------Sorted List:-------------\n");
	int treenumbs = treecount(root);
	struct tnode *list[treenumbs]; 
	addlist(list, root);
	myqsort((void **)list,0, treenumbs-1,(int (*)(void *, void *))comp_count);
	listprint(list, treenumbs);
}
...
int treecount(struct tnode *root)
{
	int count = 0;
	if (root != NULL)
	{
		count += treecount(root->leftnode);
		count++;
		count += treecount(root->rightnode);
	}
	return count;
}

struct tnode **addlist(struct tnode **list, struct tnode *root)
{
	if (root != NULL)
	{
		list = addlist(list, root->leftnode);
		
		// add node to list
		*list = root;
		list++;

		list = addlist(list, root->rightnode);
	}
	return list;
}

int comp_count(struct tnode *n1, struct tnode *n2) {
